                ...
                <div class="col-md-9">
                    <div class="d-flex justify-content-between">
                        <div class="d-flex align-items-center">
                            <h3 class="fw-bold fs-5 mb-0">Discharge Port</h3>
                                ...
                                <div class="modal-dialog rounded-5" style="max-width: 746px; width: 746px;">
                                    <div class="modal-content p-3">
                                        <div class="modal-header border-0">
                                            <h1 class="modal-title fw-bold" id="modalLabel" style="font-size: 28px">Add New Discharge Port</h1>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form action="{{ url('admin/system-configuration/discharge-port/add') }}" method="POST" class="form">
                                                @csrf
                                                <div class="row mt-4">
                        ...
                        <div class="d-flex justify-content-between mt-3 align-items-center justify-content-lg-end">
                            <div class="d-flex gap-2 align-items-center page-icon">
                                @php
                                    ...
                                    $nex = 'page='.$next;
                                @endphp
                                <a class="btn" @if(@$page == 1) href="javascript:void();" @else href="{{ url('admin/system-configuration/discharge-port?'.$pre) }}" @endif>
                                ...
                                <a class="btn" @if(count($discharge_port) < 10) href="javascript:void();" @else href="{{ url('admin/system-configuration/discharge-port?'.$nex) }}" @endif>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-fs-4">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center float-end">
                                                ...
                                                <p class="fs-5 text-danger">
                                                    <i class="fa-solid fa-circle-xmark delete" data-url="{{ url('admin/system-configuration/discharge-port/delete', @$value->id) }}" style="cursor: pointer;"></i>
                                                </p>
                                            </div>
                                        </td>
                                    </tr>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $(document).on("click", ".add", function () {
                ...
                $("#modalLabel").text("Add New Discharge Port");
                $("#name").val('');
                $("#position").val('');

                $("#modal").modal("show");
                $(".form").attr("action", "{{ url('admin/system-configuration/discharge-port/add') }}");
            ...
            $(document).on("click", ".edit", function () {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ url('admin/system-configuration/discharge-port/edit') }}/"+id,
                    success: function (res) {
                        res = JSON.parse(res);
                        console.log(res);
                        if (res.success == true) {
                            $("#modalLabel").text("Edit Discharge Port");
                            $("#name").val(res.data.name);
                            $("#position").val(res.data.position);

                            $("#modal").modal("show");
                            $(".form").attr("action", "{{ url('admin/system-configuration/discharge-port/edit') }}/"+id);
                        }
                    }
                });
            });
