                ...
                <div class="col-md-9">
                    <div class="d-flex justify-content-between">
                        <div class="d-flex align-items-center">
                            <h3 class="fw-bold fs-5 mb-0">Carriers</h3>
                                ...
                                <div class="modal-dialog rounded-5" style="max-width: 746px; width: 746px;">
                                    <div class="modal-content p-3">
                                        <div class="modal-header border-0">
                                            <h1 class="modal-title fw-bold" id="modalLabel" style="font-size: 28px">Add New Carrier</h1>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form action="{{ url('admin/system-configuration/carriers/add') }}" method="POST" class="form">
                                                @csrf
                                                <div class="row mt-4">
                                                    ...
                                                    <div class="col-md-12 mb-4">
                                                        <!-- Company -->
                                                        <div class="row">
                                                            <label for="" class="col-md-4">Phone Numbers <br><span>(Comma Seperated)</span></label>
                                                            <div class="col-md-8">
                                                                <div class="input-group shadow-lg rounded-4">
                                                                    <textarea class="form-control" name="phone_numbers" id="phone_numbers"></textarea>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                        ...
                        <div class="d-flex justify-content-between mt-3 align-items-center justify-content-lg-end">
                            <div class="d-flex gap-2 align-items-center page-icon">
                                ...
                                @endphp
                                <a class="btn" @if(@$page == 1) href="javascript:void();" @else href="{{ url('admin/system-configuration/carriers?'.$pre) }}" @endif>
                                ...
                                <a class="btn" @if(count($carriers) < 10) href="javascript:void();" @else href="{{ url('admin/system-configuration/carriers?'.$nex) }}" @endif>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-fs-4">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <form method="GET" action="{{ url('admin/system-configuration/carriers') }}" class="row align-items-center mt-3" id="filters-form">
                        <input type="hidden" name="page" value="{{ @$page }}">
                        <div class="col-md-12">
                            <input type="text" class="form-control p-2" name="search" value="{{ @$search }}" id="search-user" placeholder="Search carriers">
                        </div>
                    </form>
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <thead class="text-fs-4">
                                    ...
                                    <th scope="col" class="fw-bold">Phone Numbers</th>
                                    <th scope="col"></th>
                                </thead>
                                <tbody>
                                    @if(count(@$carriers) > 0)
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center float-end">
                                                ...
                                                <p class="fs-5 text-danger">
                                                    <i class="fa-solid fa-circle-xmark delete" data-url="{{ url('admin/system-configuration/carriers/delete', @$value->id) }}" style="cursor: pointer;"></i>
                                                </p>
                                            </div>
                                        </td>
                                    </tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on("click", ".add", function () {
                ...
                $("#modalLabel").text("Add New Carrier");
                $("#name").val('');
                $("#phone_numbers").text('');

                $("#modal").modal("show");
                $(".form").attr("action", "{{ url('admin/system-configuration/carriers/add') }}");
            ...
            $(document).on("click", ".edit", function () {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ url('admin/system-configuration/carriers/edit') }}/"+id,
                    success: function (res) {
                        res = JSON.parse(res);
                        console.log(res);
                        if (res.success == true) {
                            $("#modalLabel").text("Edit Carrier");
                            $("#name").val(res.data.name);
                            $("#phone_numbers").text(res.data.phone_numbers);

                            $("#modal").modal("show");
                            $(".form").attr("action", "{{ url('admin/system-configuration/carriers/edit') }}/"+id);
                        }
                    }
                });
            });
