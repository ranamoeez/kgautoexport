                ...
                <div class="col-md-9">
                    <div class="d-flex justify-content-between">
                        <div class="d-flex align-items-center">
                            <h3 class="fw-bold fs-5 mb-0">Notify Party</h3>
                                ...
                                <div class="modal-dialog rounded-5" style="max-width: 746px; width: 746px;">
                                    <div class="modal-content p-3">
                                        <div class="modal-header border-0">
                                            <h1 class="modal-title fw-bold" id="modalLabel" style="font-size: 28px">Add New Notify Party</h1>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form action="{{ url('admin/system-configuration/notify-party/add') }}" method="POST" class="form">
                                                @csrf
                                                <div class="row mt-4">
                        ...
                        <div class="d-flex justify-content-between mt-3 align-items-center justify-content-lg-end">
                            <div class="d-flex gap-2 align-items-center page-icon">
                                @php
                                    ...
                                    $nex = 'page='.$next;
                                @endphp
                                <a class="btn" @if(@$page == 1) href="javascript:void();" @else href="{{ url('admin/system-configuration/notify-party?'.$pre) }}" @endif>
                                ...
                                <a class="btn" @if(count($notify_party) < 10) href="javascript:void();" @else href="{{ url('admin/system-configuration/notify-party?'.$nex) }}" @endif>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-fs-4">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="mt-4">
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tbody>
                                    @if(count(@$notify_party) > 0)
                                    @foreach(@$notify_party as $key => $value)
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center float-end">
                                                ...
                                                <p class="fs-5 text-danger">
                                                    <i class="fa-solid fa-circle-xmark delete" data-url="{{ url('admin/system-configuration/notify-party/delete', @$value->id) }}" style="cursor: pointer;"></i>
                                                </p>
                                            </div>
                                        </td>
                                    </tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on("click", ".add", function () {
                ...
                $("#modalLabel").text("Add New Notify Party");
                $("#name").val('');
                $("#position").val('');
                $("#selected").attr('checked', false);

                $("#modal").modal("show");
                $(".form").attr("action", "{{ url('admin/system-configuration/notify-party/add') }}");
            ...
            $(document).on("click", ".edit", function () {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ url('admin/system-configuration/notify-party/edit') }}/"+id,
                    success: function (res) {
                        res = JSON.parse(res);
                        console.log(res);
                        if (res.success == true) {
                            $("#modalLabel").text("Edit Notify Party");
                            ...
                            $("#modal").modal("show");
                            $(".form").attr("action", "{{ url('admin/system-configuration/notify-party/edit') }}/"+id);
                        }
                    }
                });
            });
